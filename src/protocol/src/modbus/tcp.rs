...
use super::{
    decode_u16, encode_u16, pdu::encode_read_coils, Context, FunctionCode, ModbusError,
    ProtocolError,
};
    ...
    T: AsyncReadExt + AsyncWriteExt + Unpin + Send,
{
    async fn read_coils(
        ...
        if let Err(e) = self.decode_adu() {
            return Err(ModbusError::Protocol(e));
        }

        Ok(decode_read_coils(&mut self.buffer[7..]))
    }
