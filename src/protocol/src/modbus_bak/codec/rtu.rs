...
use super::*;
use crate::modbus_bak::frame::rtu::{Header, RequestAdu, ResponseAdu};
    ...
    F: Fn(&BytesMut) -> Result<Option<usize>>,
{
    ...
    for _i in 0..MAX_RETRIES {
        ...
        if let Err(err) = result {
            warn!("Failed to decode {pdu_type} frame: {err}");
    ...
    error!("Giving up to decode frame after {MAX_RETRIES} retries");
