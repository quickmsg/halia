...
impl<'de> Deserialize<'de> for MessageValue {
    #[inline]
    fn deserialize<D>(deserializer: D) -> Result<MessageValue, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for ValueVisitor {
            ...
            #[inline]
            fn visit_bool<E>(self, value: bool) -> Result<MessageValue, E> {
                Ok(MessageValue::Bool(value))
            }

            #[inline]
            fn visit_i64<E>(self, value: i64) -> Result<MessageValue, E> {
                Ok(MessageValue::Number(value.into()))
            }

            #[inline]
            fn visit_u64<E>(self, value: u64) -> Result<MessageValue, E> {
                Ok(MessageValue::Number(value.into()))
            }

            #[inline]
            fn visit_f64<E>(self, value: f64) -> Result<MessageValue, E> {
                Ok(MessageValue::Float64(value))
            }
            ...
            #[inline]
            fn visit_none<E>(self) -> Result<MessageValue, E> {
                Ok(MessageValue::Null)
            }

            #[inline]
            fn visit_some<D>(self, deserializer: D) -> Result<MessageValue, D::Error>
            where
                D: serde::Deserializer<'de>,
            {
            ...
            #[inline]
            fn visit_unit<E>(self) -> Result<MessageValue, E> {
                Ok(MessageValue::Null)
            }

            #[inline]
            fn visit_seq<V>(self, mut visitor: V) -> Result<MessageValue, V::Error>
            where
                V: SeqAccess<'de>,
            {
                let mut vec = Vec::new();
...
impl FromStr for MessageValue {
    type Err = Error;
    fn from_str(s: &str) -> Result<MessageValue, Error> {
...
macro_rules! deserialize_number {
    ($method:ident) => {
        ...
        {
            match self {
                MessageValue::Number(n) => n.deserialize_any(visitor),
